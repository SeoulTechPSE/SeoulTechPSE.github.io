...
def f1(x, L):
    """ f(x) = 0 for 0 < x < L/2; L -x for L /2 < x < L """
    f = np.zeros_like(x)
    mask1 = (x > L/2) & (x < L)
    f[mask1] = L - x[mask1]
    return f
...
def f2(x, L):
    """ f(x) = x for 0 < x < L/2; 0 for L /2 < x < L """
    f = np.zeros_like(x)
    mask1 = (x > 0) & (x < L/2)
    f[mask1] = x[mask1]
    return f
...
def make_extensions(f, L, x):
    """Return even, odd, and periodic extensions"""
    f_even = f(np.abs(x) % L, L)
    f_odd = np.sign(x) *f(np.abs(x) % L, L)
    f_periodic = f(x % L, L)
    return f_even, f_odd, f_periodic
...
# Compute extensions
f1_even, f1_odd, f1_periodic = make_extensions(f1, L1, x1)
f2_even, f2_odd, f2_periodic = make_extensions(f2, L2, x2)
...
# Case A: f1
for i, data in enumerate([f1_even, f1_odd, f1_periodic]):
    axes[0, i].plot(x1, data, 'b')
    axes[0, i].set_ylim([-0.52, 0.52])
    axes[0, i].set_title(f"Case A: {titles[i]}")
    axes[0, i].grid(True)

# Case B: f2
for i, data in enumerate([f2_even, f2_odd, f2_periodic]):
    axes[1, i].plot(x2, data, 'r')
    axes[1, i].set_ylim([-0.52, 0.52])
    axes[1, i].set_title(f"Case B: {titles[i]}")
    axes[1, i].grid(True)
