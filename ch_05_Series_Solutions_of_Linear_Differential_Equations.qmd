...
$$\displaystyle xy'' +(1-x) y' +\alpha y=0$$
...
$$\displaystyle L_n(x) = \frac{e^x}{n!} \frac{d^n}{dx^n} x^n e^{-x}$$
...
$$\displaystyle \color{blue}{y(x) = x^r \sum_{k=0}^{\infty} a_k x^k = \sum_{k=0}^{\infty} a_k x^{k + r}, \quad a_0 \ne 0}$$

Then,

$$\displaystyle y'(x) = \sum_{k=0}^{\infty} a_k (k + r) x^{k + r - 1}$$
...
$$\displaystyle \sum_{k=0}^{\infty} \left[ a_{k+1} (k + 1)^2 + a_k (\alpha - k) \right] x^k = 0$$

So the recurrence relation is:

$$\displaystyle \color{blue}{a_{k+1} = - \frac{(\alpha - k)}{(k + 1)^2} a_k}$$

Choose $a_0 = 1$ (arbitrary constant), this gives a general form:

$$\displaystyle \color{blue}{a_k = \frac{(-1)^k \alpha(\alpha - 1) \cdots (\alpha - k + 1)}{(k!)^2}}
$$

Thus the first solution is

$$\displaystyle \color{blue}{y_1(x) = \sum_{k=0}^{\infty} a_k x^k}$$

When the indicial equation has a repeated root $r=0$, the second solution has the form:

$$\displaystyle \color{blue}{y_2(x) = y_1(x) \ln x + \sum_{k=0}^{\infty} b_k x^k}$$

We plug this into the original equation. This leads to 

$$\displaystyle 2y_1' -y_1 +\sum_{k=0}^\infty \left[ (k+1)^2 b_{k+1} +(\alpha -k) b_k \right] x^k = 0$$
...
$$\displaystyle \color{blue}{y_1(x) = \sum_{k=0}^{n} \frac{(-1)^k n!}{(n - k)! (k!)^2} x^k = L_n(x)}$$

The second linearly independent solution is of the form:

$$\displaystyle y_2(x) = L_n(x) \ln x + \sum_{k=0}^{\infty} d_k x^k$$

**Solution (c)**

To find the Laguerre polynomials $L_n(x)$ for $n = 0, 1, 2, 3, 4$, we will use Rodrigues’ formula.

$$\displaystyle \color{blue}{L_0(x) = \frac{e^x}{0!} \frac{d^0}{dx^0} \left( x^0 e^{-x} \right) = e^x \cdot e^{-x} = 1}$$

$$\displaystyle \color{blue}{L_1(x) = \frac{e^x}{1!} \frac{d}{dx} \left( x e^{-x} \right) = e^x \cdot (1 - x) e^{-x} = 1 - x}$$

$$\displaystyle \color{blue}{L_2(x) = \frac{e^x}{2!} \frac{d^2}{dx^2} \left( x^2 e^{-x} \right) = 1 - 2x + \frac{1}{2}x^2}$$

$$\displaystyle \color{blue}{L_3(x) = 1 - 3x + \frac{3}{2}x^2 - \frac{1}{6}x^3}$$

$$\displaystyle \color{blue}{ L_4(x) = 1 - 4x + 3x^2 - \frac{2}{3}x^3 + \frac{1}{24}x^4}$$
...
$$\displaystyle T_n(x) = (-1)^n \frac{2^n n!}{(2n)!} \sqrt{1-x^2}\frac{d^n}{dx^n} (1-x^2)^{n-1/2}$$
...
$$\displaystyle a_{k+2} = -\frac{n^2 - k^2}{(k+2)(k+1)} a_k$$

Observe that when $\color{blue}{k = n}$, we get:

$$\displaystyle \color{blue}{a_{n+2} = -\frac{n^2 - n^2}{(n+2)(n+1)} a_n = 0}$$
...
$$\displaystyle f_n(x) = (1 - x^2)^{n - \frac{1}{2}}, \quad \text{then} \quad T_n(x) = (-1)^n \frac{2^n n!}{(2n)!} \sqrt{1 - x^2} \cdot \frac{d^n}{dx^n} f_n(x)$$
...
* $n = 0$
    ...
    > $T_0(x) = \frac{1}{1} \cdot \sqrt{1 - x^2} \cdot (1 - x^2)^{-1/2} = 1$
...
* $n = 2$
    ...
    >$\color{red}{T_2(x)} = \frac{2^2 \cdot 2!}{(4)!} \cdot \sqrt{1 - x^2} \cdot f_2''(x)
    = \frac{4 \cdot 2}{24} \cdot \sqrt{1 - x^2} \cdot \left[ -3(1 - x^2)^{1/2} + 3x^2(1 - x^2)^{-1/2} \right] = 2x^2 - 1$
...
* $n = 3$
    ...
    > $\color{red}{T_3(x)} = (-1)^3 \cdot \frac{2^3 \cdot 3!}{(6)!} \cdot \sqrt{1 - x^2} \cdot f_3’’’(x)
    = -\frac{8 \cdot 6}{720} \cdot \sqrt{1 - x^2} \cdot \left[ 45x(1 - x^2)^{1/2} - 15x^3 (1 - x^2)^{-1/2} \right] = 4x^3 - 3x$
