...
if MPI.COMM_WORLD.rank == 0:
    ...
    for idx, (eigval, deg, _) in enumerate(grouped_eigenvalues[:min(12, len(grouped_eigenvalues))]):
        if idx < len(theoretical):
            ...
            if error > 1.0:
                print(f" Mode {idx+1:2d} {mode_label}: computed = {eigval:7.3f} (×{deg}),"
                      f" theoretical = {theo_val:7.3f} (×{theo_deg}), error = {error:5.2f}% {deg_match} ⚠️")
            else:
                print(f" Mode {idx+1:2d} {mode_label}: computed = {eigval:7.3f} (×{deg}),"
                      f" theoretical = {theo_val:7.3f} (×{theo_deg}), error = {error:5.2f}% {deg_match}")
